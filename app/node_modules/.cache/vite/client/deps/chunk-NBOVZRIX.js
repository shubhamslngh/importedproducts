...
function trainCase(str, opts) {
  return (Array.isArray(str) ? str : splitByCase(str)).filter(Boolean).map((p) => upperFirst((opts == null ? void 0 : opts.normalize) ? p.toLowerCase() : p)).join("-");
}
...
function titleCase(str, opts) {
  return (Array.isArray(str) ? str : splitByCase(str)).filter(Boolean).map(
    (p) => titleCaseExceptions.test(p) ? p.toLowerCase() : upperFirst((opts == null ? void 0 : opts.normalize) ? p.toLowerCase() : p)
  ).join(" ");
}
...
//# sourceMappingURL=chunk-NBOVZRIX.js.map
